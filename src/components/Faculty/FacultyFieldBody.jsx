...
const AnnouncementFieldBody = ({
  ...
  useEffect(() => {
    if (formData.a_img) {
      fetch(`${API_ENDPOINTS.getImages}`)
        .then((res) => res.json())
        .then((result) => {
          const matched = result.data.find((img) => img.image_id === formData.a_img);
          if (matched) {
  ...
  useEffect(() => {
    ...
    if (typeof formData.a_fav !== 'boolean') {
      setFormData((prev) => ({
        ...prev,
        a_fav: !!parseInt(prev.a_fav),
      }));
    }

    if (formData.a_date && formData.a_date.includes(' ')) {
      const dateOnly = formData.a_date.split(' ')[0];
      setFormData((prev) => ({
        ...prev,
        a_date: dateOnly,
      }));
          ...
          <div className="grid grid-cols-1 md:!grid-cols-2 gap-4 py-2">
            <div className="flex-1">
              ...
              <div className="mt-2">
                <input
                  type="text"
                  value={formData.a_shorttitle || ''}
                  onChange={(e) =>
                    setFormData((prev) => ({
                      ...prev,
                      a_shorttitle: e.target.value,
                    }))
                  }
                  className="block w-full !border-gray-200 border-0 rounded-md py-2 pl-5 text-gray-900 shadow-sm ring-1 ring-inset !ring-gray-300 placeholder:text-gray-400 focus:ring-2 sm:text-2xl sm:leading-6"
                />
            ...
            <div className="flex-1">
              ...
              <div className="mt-2">
                <select
                  value={formData.a_fav}
                  onChange={(e) =>
                    setFormData((prev) => ({
                      ...prev,
                      a_fav: e.target.value === 'true',
                    }))
                  }
                  className="block w-full !border-gray-200 border-0 rounded-md py-2 pl-5 text-gray-900 shadow-sm ring-1 ring-inset !ring-gray-300 placeholder:text-gray-400 focus:ring-2 sm:text-2xl sm:leading-6"
                >
