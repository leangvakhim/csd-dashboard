...
const RequirementPiece = forwardRef(({sectionId, pageId, handleSectionRef}, ref) => {
  ...
  useEffect(() => {
    ...
    const fetchSubRequirements = async (reqId) => {
      try {
        const response = await axiosInstance.get(`${API_ENDPOINTS.getSubRequirement}?gca_gc=${reqId}`);
        const subrequirements = response.data.data || [];

        if (subrequirements.length > 0) {
          const sectionResponse = await axiosInstance.get(`${API_ENDPOINTS.getSection}/${sectionId}`);
        ...
        <summary className="cursor-pointer flex justify-between rounded-lg py-2 w-full ">
          ...
          >
            <div className="flex gap-1 items-center" ref={handleSectionRef}>
              ...
              <span className=" text-xl font-medium">Requirement</span>
            </div>
