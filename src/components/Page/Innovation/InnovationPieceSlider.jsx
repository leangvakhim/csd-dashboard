...
const InnovationPieceOne = forwardRef(({ innovationId }, ref) => {
  ...
  useImperativeHandle(ref, () => ({
    getSubserviceSlidersInnovation: async () => {
  ...
  useEffect(() => {
    const fetchSliders = async () => {
      try {
        ...
        if (subservices.length > 0 && innovationId) {
          const validSubservices = subservices.filter(
            (item) => item.ss_ras === innovationId
          );
