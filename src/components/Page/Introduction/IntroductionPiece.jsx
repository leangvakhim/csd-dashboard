    ...
    const fetchIntroductions = async () => {
      try {
        const response = await axios.get(
          `${API_ENDPOINTS.getIntroduction}?in_sec=${sectionId}`
        );
        ...
        if (introductions.length > 0) {
          ...
          if (introduction) {
            ...
            setSelectedImage(
              introduction.in_img
                ? `${API}/storage/uploads/${introduction.image.img}`
                : ""
            );
          }
        }
...
>>>>>>> Stashed changes
        ...
        <div className="w-full flex flex-col sm:!flex-row gap-6 px-4 py-2">
