    ...
    const fetchApplys = async () => {
      try {
        const response = await axios.get(
          `${API_ENDPOINTS.getApply}?ha_sec=${sectionId}`
        );
        const applys = response.data.data || [];
        if (applys.length > 0) {
          const apply = applys.find(
            (item) =>
              item.section.sec_page === pageId && item.ha_sec === sectionId
          );
          ...
          if (apply) {
            ...
            setSelectedImage(
              apply.ha_img ? `${API}/storage/uploads/${apply.image.img}` : ""
            );
          }
        }
        ...
        <div className="grid grid-cols-1 gap-4 ">
            <details className="group [&_summary::-webkit-details-marker]:hidden rounded-lg">
                <summary className="cursor-pointer flex justify-between rounded-lg py-2 w-full ">
                    ...
                    >
                        <div className="flex gap-1 items-center">
                            ...
                            <span className=" text-xl font-medium">Apply</span>
                        </div>
                ...
                <div className="grid grid-cols-1 sm:!grid-cols-2 gap-4 px-4 py-2 ">
                    ...
                    <div className="flex flex-col gap-2 px-4 space-y-2">
                        <div className="flex-1">
                            <label className="block text-xl font-medium leading-6 text-white-900">
                                Tag title
                            </label>
