...
const CarouselPieceSlider = forwardRef(
  ({ displaySlideshow, sectionId, pageId }, ref) => {
      ...
      const fetchSliders = async () => {
        try {
          const response = await axios.get(
            `${API_ENDPOINTS.getSlideshow}?ban_sec=${sectionId}`
          );
          const slideshows = response.data?.data || [];

          if (slideshows.length > 0) {
            const validSlideshows = slideshows.filter(
              (item) => item.slider_sec.sec_page === pageId
            );
...
=======
            const formattedData = validSlideshows.map((item) => ({
              ...
              subtitle: item.slider_text || "",
              logo: item.logo?.img
                ? `${API}/storage/uploads/${item.logo.img}`
                : null,
              image: item.img?.img
...
>>>>>>> Stashed changes
    ...
    const handleDeleteSlider = async ({ sliderId }) => {
      ...
      if (result.isConfirmed) {
        try {
          await axios.put(`${API_ENDPOINTS.deleteSection}/${sliderId}`);
                              ...
                              <details className=" group [&_summary::-webkit-details-marker]:hidden border-2 rounded-lg grid-cols-1">
                                <summary className="cursor-pointer flex justify-between rounded-lg py-2 w-full ">
                                  <div
                                    ...
                                    onClick={() =>
                                      setIsRotatedButton1(!isRotatedButton1)
                                    }
                                  >
                              ...
                              <details className=" group [&_summary::-webkit-details-marker]:hidden border-2 rounded-lg grid-cols-1">
                                <summary className="cursor-pointer flex justify-between rounded-lg py-2 w-full ">
                                  <div
                                    className="cursor-pointer flex items-center justify-between w-full px-4"
                                    onClick={() =>
                                      setIsRotatedButton2(!isRotatedButton2)
                                    }
                                  >
                                    <span className=" text-xl font-medium">
                                    ...
                                    <div className="mt-2">
                                      <label class="toggle-switch mt-2">
                                        <input
                                          type="checkbox"
                                          checked={
                                            sliders.secondbtndisplay || false
                                          }
                                          onChange={(e) =>
                                            handleInputChange(
                                              sliders.id,
